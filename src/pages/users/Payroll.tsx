...
import { mockSalaryHistory, type SalarySlip } from "@/data/mock-payroll";
...
const Payroll: React.FC = () => {
    ...
    const generatePDFContent = (doc: jsPDF, slip: SalarySlip) => {
        ...
        doc.setFontSize(10);
        doc.setTextColor(0);
        doc.text(`Employee Name: Sujay Kumar`, 15, 75);
    ...
    const exportToCSV = () => {
        ...
        // Rows
        const rows = mockSalaryHistory.map(slip => [
            slip.id,
            slip.month,
            slip.date,
            slip.basicSalary,
    ...
    return (
        <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 p-6 lg:p-10 animate-in fade-in duration-500">
            <div className="max-w-6xl mx-auto space-y-8">
                ...
                {/* --- Header --- */}
                <div className="sticky top-0 z-20 bg-slate-50/95 dark:bg-slate-950/95 backdrop-blur support-[backdrop-filter]:bg-slate-50/50 py-4 -mx-6 px-6 lg:-mx-10 lg:px-10 -mt-6 lg:-mt-10 border-b border-slate-200/50 dark:border-slate-800/50 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <Card className="p-6 border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900/50 flex flex-col justify-between">
                        ...
                        <div>
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{formatCurrency(2800)}</h2>
                            <p className="text-xs text-green-600 dark:text-green-400 mt-1 flex items-center">
                                <TrendingUp className="h-3 w-3 mr-1" /> Paid on Dec 31
                            </p>
                        </div>
                    </Card>
                    ...
                    <Card className="p-6 border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900/50 flex flex-col justify-between">
                        ...
                        <div>
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white">{formatCurrency(1200)}</h2>
                    ...
                    <Card className="p-6 border-slate-200 dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900/50 flex flex-col justify-between">
                        ...
                        <div>
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white">Jan 31</h2>
                            <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">
                                In 8 days
                            </p>
                        </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                                        ...
                                        {mockSalaryHistory.map((slip) => (
                                            ...
                                            >
                                                <td className="px-3 py-4">
                                                    <div className="font-medium text-slate-900 dark:text-white text-sm">{slip.month.split(" ")[0]} <span className="hidden sm:inline">{slip.month.split(" ")[1]}</span></div>
                    ...
                    <div className="xl:col-span-1">
                        {selectedSlip ? (
                            <Card className="border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900 relative overflow-hidden">
                                ...
                                <div className="p-6 space-y-6">
                                    ...
                                    <Button
                                        className="w-full bg-green-500 hover:bg-green-600 text-white dark:bg-slate-800 dark:text-white dark:hover:bg-slate-700 cursor-pointer"
                                        onClick={() => generatePayslip(selectedSlip)}
                                    >
                                        <Download className="mr-2 h-4 w-4" /> Download Payslip PDF
                                    </Button>
                                </div>
                            </Card>
                        ) : (
                            <div className="h-64 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl flex flex-col items-center justify-center text-center p-6 text-slate-400 dark:text-slate-500">
